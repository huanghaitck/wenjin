...
当前书架包括原始史料、学术论文、学术专著、个人论文与稿件、读书笔记、工具书与目录、待分类。读书笔记默认不进入知识图谱，待分类文件在完成书目判断前也不进入；只有用户明确要求时，Agent/API才在当前查询中临时纳入读书笔记。作品关系、书目实体与项目内容实体使用各自的节点、关系和图例，不再混用。

下面是个人测试库生成的界面示例；节点数量随用户实际入库和分类情况变化。
