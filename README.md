...
当前版本为 **0.1.2 Public Preview**，主要支持 Windows 10/11。
...
普通用户从[0.1.2发布页](https://github.com/huanghaitck/wenjin/releases/tag/v0.1.2)下载 `wenjin-0.1.2-x64-setup.exe` 并运行即可。安装包已经包含问津所需组件，不需要另装Python、Node.js、PowerShell 7或Rust。Windows 10/11通常已有WebView2；断网机器可下载包含WebView2安装程序的完整离线包。研究浏览器优先使用系统Microsoft Edge，也可以指定兼容的Chrome/Chromium。
...
当前书架包括原始史料、学术论文、学术专著、个人论文与稿件、读书笔记、工具书与目录、待分类。读书笔记不进入书目关系图谱。
...
问津核心程序不绑定具体研究对象。0.1.2中的领域包可以成为具有独立线程和隔离记忆的领域 Agent，并增加专门研究Skill、受权限约束的MCP工具和本地数据连接。它还可分别声明领域推理、主视觉、二次视觉复核和备用模型岗位，由问津凭据库在运行时注入，不要求用户编辑领域包。
...
0.1.2 仅支持由对方发起的私聊文字消息。群聊、主动或定时推送、文件收发、付款、验证码代填均未开放。需要执行工具时仍服从“请求批准、帮我批准、完全访问”三档权限；等待审批的动作只会在微信中提示回到问津客户端处理。

## 平台支持

0.1.2目前只发布Windows 10/11 x64版本。macOS版本尚未发布：当前的电脑控制和安全凭据组件使用Windows接口，必须在Mac上另行适配、构建和测试后才能提供下载，不能直接把未经测试的程序标成Mac版。
