...
当前候选版本为 **0.1.3 RC1**，主要支持 Windows 10/11。
...
普通用户可从发布页下载 Windows 安装程序。安装包已经包含问津核心运行时，不需要另装 Python、Node.js、PowerShell 7 或 Rust；自包含领域 Agent 也不依赖系统 Python。只有明确运行外部脚本时才需要可选脚本环境，主 Agent 会先自检，并在用户批准后按需安装。Windows 10/11通常已有WebView2；断网机器可下载包含WebView2安装程序的完整离线包。
