...
class WenjinV01Tests(unittest.TestCase):
    ...
    def test_ui_separates_agent_configuration_and_draws_library_graph(self) -> None:
        ...
        self.assertIn("['same_author','same_journal','same_publisher','cites'", script)
        self.assertIn("&shelf=", script)
        self.assertIn("data-legend-relation", script)
        self.assertIn("directedRelations", script)
        self.assertIn("swatch.textContent='⇢'", script)
        for relation in ("cites", "uses_material_from", "reviews", "translates", "mentions_work"):
            self.assertRegex(script, rf'edge\[relation = "{relation}"\].+?line-style\':\'dashed\'.+?target-arrow-shape\':\'triangle\'')
        self.assertIn("layoutElements=sparseLargeGraph", script)
