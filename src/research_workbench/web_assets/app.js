...
function renderKnowledgeGraph(data){
  ...
  const workIds={};for(const edge of data.edges){if(byId[edge.source_node_id]?.node_type==='work')workIds[edge.source_node_id]=edge.work_id;}
  const receipt=document.createElement('p');receipt.className='boundary-note';receipt.textContent=`${data.node_count} 个节点 · ${data.edge_count} 条关系。图谱来自已登记的书目、作者、年代、出版者、书架和人工标签。`;graph.append(receipt);
  if(!data.nodes.length){graph.append(Object.assign(document.createElement('p'),{className:'empty',textContent:'当前检索范围没有可绘制的书目关系。'}));return;}
  const typeOrder=['work','person','year','organization','material_type','tag'];
  const typeLabels={work:'作品',person:'作者',year:'年代',organization:'出版者',material_type:'材料类型',tag:'书架/标签'};
...
async function setLibraryView(view){
  ...
  try{const data=await request(`/api/library/graph?query=${encodeURIComponent($('libraryQuery').value.trim())}&limit=24`);renderKnowledgeGraph(data);notice(`知识图谱已按当前检索范围绘制${data.backfilled_work_count?`，并补建 ${data.backfilled_work_count} 部旧书目的关系`:''}。`);}catch(error){notice(error.message,true);}
}
...
function renderSettings() {
  ...
  const pluginInstall=card(english?'Install a domain pack':'安装领域包',english?'A domain pack may contribute research methods, field schemas, Agent tools, document processors, graph adapters, and declarations for local data sources. Install a versioned folder or ZIP that contains a Wenjin manifest.':'领域包可以扩展研究方法、字段规范、Agent工具、资料处理器、知识图谱适配器和本地数据接口。载入含问津清单的版本化目录或ZIP即可安装。');
  ...
  appendSetting('plugins',card(english?'What a domain pack contains':'领域包至少包含什么',english?'1. A clear research scope, material types, methods, fields, and stopping rules. 2. A Skill or domain profile used by the main Agent. 3. Bounded MCP tools with permission classes. 4. Optional processors, schemas, graph adapters, and contextual panels routed into existing Wenjin workspaces. 5. Bundled data or declarations for user-selected local data. Version, compatibility, and license metadata remain available under engineering information instead of defining the pack in the main interface. A separate acting persona is optional and is not created by default.':'1. 明确的研究范围、材料类型、方法、字段和停止条件。2. 供主Agent使用的Skill或领域配置。3. 带权限等级的有界MCP工具。4. 按需接入现有工作区的资料处理器、字段规范、知识图谱适配器和上下文面板。5. 随包数据，或由用户选择的本地数据接口。版本、兼容范围和许可证保留在工程信息中，不再作为主界面对领域包的首要说明。独立行动人格不是必需项，默认不会创建。'));
  ...
  if(!(pluginState.plugins||[]).length)appendSetting('plugins',card(english?'No domain pack installed':'尚未安装领域包',english?'Domain packs extend specialist methods, schemas, tools, processors, graph adapters, and local-data connectors without changing the Wenjin core.':'领域包用于扩展专门研究方法、字段规范、工具、资料处理器、知识图谱适配器和本地数据接口；问津核心保持独立。'));
