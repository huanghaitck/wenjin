...
function renderScan() {
  ...
  for (const candidate of scan.candidates.filter((item) => item.triage_state !== 'unsupported')) {
    ...
    const meta = document.createElement('small'); meta.textContent = `${actionLabels[candidate.proposed_action] || candidate.proposed_action} · ${triageLabels[candidate.triage_state] || candidate.triage_state} · 建议书架：${candidate.suggested_shelf_label || '待分类'} · ${candidate.format.toUpperCase()} · ${formatBytes(candidate.byte_count)}`;
...
function renderInteractiveGraph(container,nodes,edges,{background='#f2eee6',onOpen=()=>{},className=''}={}){
  ...
  const toolbar=document.createElement('div');toolbar.className='graph-toolbar';toolbar.append(Object.assign(document.createElement('span'),{textContent:state.language==='en'?'Wheel to zoom · drag background or middle-drag to pan · hover for title · double-click for details':'滚轮缩放 · 拖动空白处或中键平移 · 悬停看题名 · 双击看详情'}));
  ...
  let layoutName='cose';try{if(window.cytoscape('layout','fcose'))layoutName='fcose';}catch(_error){}
  const layoutOptions=layoutName==='fcose'?{name:'fcose',quality:nodes.length>140?'draft':'default',randomize:true,animate:false,fit:false,padding:60,packComponents:false,nodeSeparation:nodes.length>140?55:80,nodeRepulsion:node=>nodes.length>140?5200:7000,idealEdgeLength:edge=>edges.length>nodes.length?70:95,edgeElasticity:edge=>.35,numIter:nodes.length>140?1500:2500,tile:true,tilingPaddingVertical:22,tilingPaddingHorizontal:22}:{name:'cose',animate:false,fit:false,padding:60,randomize:true,nodeRepulsion:90000,idealEdgeLength:120,componentSpacing:120};
  const controls=document.createElement('div');controls.className='graph-zoom-controls';for(const [label,title,run] of [['＋',state.language==='en'?'Zoom in':'放大',()=>cy.zoom({level:Math.min(cy.maxZoom(),cy.zoom()*1.25),renderedPosition:{x:stage.clientWidth/2,y:stage.clientHeight/2}})],['－',state.language==='en'?'Zoom out':'缩小',()=>cy.zoom({level:Math.max(cy.minZoom(),cy.zoom()/1.25),renderedPosition:{x:stage.clientWidth/2,y:stage.clientHeight/2}})],['⤢',state.language==='en'?'Fit graph':'适应画布',()=>cy.fit(undefined,45)]]){const button=document.createElement('button');button.type='button';button.textContent=label;button.title=title;button.onclick=run;controls.append(button);}toolbar.append(controls);container.append(toolbar,stage);
  const cy=window.cytoscape({container:stage,minZoom:.2,maxZoom:4,userZoomingEnabled:true,userPanningEnabled:true,elements:[...nodes.map((node)=>{const short=node.label.length>22?`${node.label.slice(0,21)}…`:node.label;return {data:{id:node.node_id,label:node.label,short,display:'',type:node.node_type,category:node.graph_category||node.node_type,raw:node}};}),...edges.map((edge,index)=>({data:{id:`edge-${index}`,source:edge.source_node_id,target:edge.target_node_id,label:edge.relation||'',relation:edge.relation||''}}))],layout:{name:'preset'},style:[
  ...
  const connected=cy.nodes().filter((node)=>node.degree()>0),isolated=cy.nodes().filter((node)=>node.degree()===0);
  const placeIsolated=()=>{const box=connected.length?connected.boundingBox():{x1:-40,y1:-40,w:80,h:80};const center={x:box.x1+box.w/2,y:box.y1+box.h/2},base=Math.max(90,Math.max(box.w,box.h)/2+80),golden=Math.PI*(3-Math.sqrt(5));isolated.forEach((node,index)=>{const seed=[...node.id()].reduce((value,char)=>(value*33+char.charCodeAt(0))>>>0,5381);const angle=index*golden+(seed%997)/997*.35,radius=base+34*Math.sqrt(index+1);node.position({x:center.x+Math.cos(angle)*radius*1.35,y:center.y+Math.sin(angle)*radius*.86});});cy.fit(cy.elements(),55);};
  if(connected.length){const components=connected.union(connected.connectedEdges()).components(),golden=Math.PI*(3-Math.sqrt(5));let finished=0;for(const [index,component] of components.entries()){const options={...layoutOptions,quality:component.nodes().length>140?'draft':'default'},layout=component.layout(options);layout.one('layoutstop',()=>{const componentNodes=component.nodes();if(componentNodes.some((node)=>!Number.isFinite(node.position('x'))||!Number.isFinite(node.position('y')))){componentNodes.forEach((node,nodeIndex)=>{const angle=nodeIndex*2*Math.PI/Math.max(componentNodes.length,1);node.position({x:Math.cos(angle)*70,y:Math.sin(angle)*70});});}const box=componentNodes.boundingBox(),angle=index*golden,radius=130*Math.sqrt(index),target={x:Math.cos(angle)*radius,y:Math.sin(angle)*radius*.72};componentNodes.shift({x:target.x-(box.x1+box.w/2),y:target.y-(box.y1+box.h/2)});finished+=1;if(finished===components.length)placeIsolated();});layout.run();}}else placeIsolated();
...
function renderKnowledgeGraph(data){
  ...
  const legend=document.createElement('div');legend.className='graph-color-legend';
  const presentCategories=new Set(data.nodes.map((node)=>node.graph_category));
  const categoryLabels=english?{primary_sources:'Primary sources',academic_articles:'Articles',monographs:'Monographs',personal_manuscripts:'Personal papers',reference_works:'Reference works',unclassified:'Unclassified'}:{primary_sources:'原始史料',academic_articles:'期刊论文',monographs:'学术专著',personal_manuscripts:'本人文章与稿件',reference_works:'工具书',unclassified:'待分类'};
  for(const [key,label] of Object.entries(categoryLabels))if(presentCategories.has(key))legend.append(Object.assign(document.createElement('span'),{className:`legend-node legend-${key}`,textContent:label}));
  const presentRelations=new Set(data.edges.map((edge)=>edge.relation));
  const relationLegend=english?{same_author:'Same author',same_publisher:'Same publisher',same_journal:'Same journal',same_decade:'Same decade',cites:'Citation',uses_material_from:'Material use',reviews:'Review',translates:'Translation',mentions_work:'Mention'}:{same_author:'同作者',same_publisher:'同出版社',same_journal:'同期刊',same_decade:'同年代（十年）',cites:'引用',uses_material_from:'使用其材料',reviews:'评述',translates:'翻译',mentions_work:'提及'};
  for(const [key,label] of Object.entries(relationLegend))if(presentRelations.has(key))legend.append(Object.assign(document.createElement('span'),{className:`legend-edge legend-${key}`,textContent:label}));graph.append(legend);
...
async function setLibraryView(view){
  ...
  try{const graphQuery=$('libraryQuery').value.trim();const graphLimit=graphQuery?80:500;const data=await request(`/api/library/graph?query=${encodeURIComponent(graphQuery)}&limit=${graphLimit}`);renderKnowledgeGraph(data);notice(state.language==='en'?`Knowledge graph shows ${(data.work_cards||[]).length} work(s) in the current search.`:`知识图谱显示当前检索范围内的${(data.work_cards||[]).length}部作品。`);}catch(error){notice(error.message,true);}
}
...
function renderSettings() {
  ...
  const pluginInstall=card(english?'Install a domain pack':'安装领域包',english?'Version 0.1.2 activates the pack Skill, allowlisted MCP tools, and user-selected local-data bindings. Schema, processor, graph-adapter, and panel entries are retained as engineering declarations for future integration and are not loaded automatically.':'0.1.2会启用领域包的Skill、清单允许的MCP工具和用户选择的本地数据连接。字段规范、资料处理器、图谱适配器和面板目前只保存为工程声明，不会自动载入现有工作区。');
  const pluginPath=document.createElement('input');pluginPath.placeholder=english?'Domain-pack folder or .zip package':'领域包目录或 .zip 安装包';
...
$('searchLibrary').onclick = async () => {
  try {
    ...
    renderWorkList();if(state.libraryView==='graph')await setLibraryView('graph');notice(`找到 ${state.libraryWorks.length} 部作品。`);
  } catch (error) { notice(error.message, true); }
};
...
$('approveAllCandidates').onclick = async () => {
  ...
  try{
    ...
    await loadLibraryScan(state.libraryScan.session_id,state.libraryScanPage);
    if(result.approved[0])state.libraryWorkId=result.approved[0].work_id;
    await refreshLibrary();notice(`已按建议分类入库 ${result.approved.length} 个候选；原文件没有移动或修改。`);
  }catch(error){notice(error.message,true);}
