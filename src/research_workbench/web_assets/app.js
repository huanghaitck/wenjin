...
function renderSettings() {
  ...
  const renderRole=(item)=>{
    const routeState=item.direct_route?(english?'Used by its named Wenjin workflow.':'由对应问津功能直接调用。'):(english?'Reserved in 0.1.1: not routed automatically; available only when selected as a MoA adviser.':'0.1.1预留：不会自动路由，只能在选为MoA参考模型时参与建议。');
  ...
  const weixin=card(english?'Weixin connection · Experimental':'微信连接 · 实验功能',english?'Scan with ordinary Weixin to try a private research conversation with this Wenjin installation. Version 0.1.1 replies only to inbound private text. Group chat, scheduled messages, files, payments, and CAPTCHA handling are not available.':'使用普通微信扫码后，可以试用从微信继续本机问津私聊研究对话。0.1.1只回复收到的私聊文字，暂不支持群聊、定时消息、文件、支付或验证码处理。');
  ...
  const pluginInstall=card(english?'Install a domain pack':'安装领域包',english?'Version 0.1.1 activates the pack Skill, allowlisted MCP tools, and user-selected local-data bindings. Schema, processor, graph-adapter, and panel entries are retained as engineering declarations for future integration and are not loaded automatically.':'0.1.1会启用领域包的Skill、清单允许的MCP工具和用户选择的本地数据连接。字段规范、资料处理器、图谱适配器和面板目前只保存为工程声明，不会自动载入现有工作区。');
  ...
  if(!(pluginState.plugins||[]).length)appendSetting('plugins',card(english?'No domain pack installed':'尚未安装领域包',english?'In 0.1.1 a domain pack can add a research Skill, permission-bounded MCP tools, and local-data bindings without changing Wenjin core.':'0.1.1中的领域包可以增加研究Skill、受权限约束的MCP工具和本地数据连接，而不改变问津核心。'));
